/*!
 * tcCore JavaScript Library v1.10.2
 * http://tcCore.com/
 *
 * Includes Sizzle.js
 * http://sizzlejs.com/
 *
 * Copyright 2005, 2013 tcCore Foundation, Inc. and other contributors
...
var
	...
	tcCore = function( selector, context ) {
		...
		return new tcCore.fn.init( selector, context, roottcCore );
	},
...
tcCore.fn = tcCore.prototype = {
	...
	constructor: tcCore,
	init: function( selector, context, roottcCore ) {
		var match, elem;
		...
		// Shortcut for document ready
			...
			return roottcCore.ready( selector );
		}
...
tcCore.fn.init.prototype = tcCore.fn;
...
tcCore.extend({
	...
	expando: "tcCore" + ( core_version + Math.random() ).replace( /\D/g, "" ),
	...
	ready: function( wait ) {
		...
		if ( tcCore.fn.trigger ) {
			tcCore( document ).trigger("ready").off("ready");
		}
	},
...
// All tcCore objects should point back to these
...
function createOptions( options ) {
	...
	tcCore.each( options.match( core_rnotwhite ) || [], function( _, flag ) {
		object[ flag ] = true;
	});
	return object;
}
				...
				then: function( /* fnDone, fnFail, fnProgress */ ) {
					...
					return tcCore.Deferred(function( newDefer ) {
						tcCore.each( tuples, function( i, tuple ) {
							var action = tuple[ 0 ],
								fn = tcCore.isFunction( fns[ i ] ) && fns[ i ];
...
tcCore.support = (function( support ) {
	...
	tcCore(function() {
		...
		tcCore.swap( body, body.style.zoom != null ? { zoom: 1 } : {}, function() {
...
function internalData( elem, name, data, pvt /* Internal Use Only */ ){
	...
	if ( !id ) {
		...
		if ( isNode ) {
			id = elem[ internalKey ] = core_deletedIds.pop() || tcCore.guid++;
		} else {
			id = internalKey;
		}
	}
...
function internalRemoveData( elem, name, pvt ) {
	...
	/* jshint eqeqeq: false */
	} else if ( tcCore.support.deleteExpando || cache != cache.window ) {
		/* jshint eqeqeq: true */
		delete cache[ id ];
...
tcCore.extend({
	...
	acceptData: function( elem ) {
		...
		var noData = elem.nodeName && tcCore.noData[ elem.nodeName.toLowerCase() ];
...
tcCore.fn.extend({
	data: function( key, value ) {
		...
		return arguments.length > 1 ?
			...
			elem ? dataAttr( elem, key, tcCore.data( elem, key ) ) : null;
	},
...
var nodeHook, boolHook,
	...
	getSetAttribute = tcCore.support.getSetAttribute,
	getSetInput = tcCore.support.input;

tcCore.fn.extend({
	attr: function( name, value ) {
		return tcCore.access( this, tcCore.attr, name, value, arguments.length > 1 );
	},
	...
	prop: function( name, value ) {
		return tcCore.access( this, tcCore.prop, name, value, arguments.length > 1 );
	},
		...
		if ( proceed ) {
			...
			for ( ; i < len; i++ ) {
				...
				if ( cur ) {
					...
					elem.className = tcCore.trim( cur );
		...
		if ( proceed ) {
			...
			for ( ; i < len; i++ ) {
				...
				if ( cur ) {
					...
					elem.className = value ? tcCore.trim( cur ) : "";
				}
			}
		}
...
tcCore.extend({
	valHooks: {
		...
		select: {
			get: function( elem ) {
				...
				for ( ; i < max; i++ ) {
					...
					if ( ( option.selected || i === index ) &&
							...
							( tcCore.support.optDisabled ? !option.disabled : option.getAttribute("disabled") === null ) &&
			...
			set: function( elem, value ) {
				...
				while ( i-- ) {
					option = options[ i ];
					if ( (option.selected = tcCore.inArray( tcCore(option).val(), values ) >= 0) ) {
						optionSet = true;
					}
				}
	...
	attrHooks: {
		type: {
			set: function( elem, value ) {
				if ( !tcCore.support.radioValue && value === "radio" && tcCore.nodeName(elem, "input") ) {
...
tcCore.each( tcCore.expr.match.bool.source.match( /\w+/g ), function( i, name ) {
	var getter = tcCore.expr.attrHandle[ name ] || tcCore.find.attr;

	tcCore.expr.attrHandle[ name ] = getSetInput && getSetAttribute || !ruseDefault.test( name ) ?
		function( elem, name, isXML ) {
			var fn = tcCore.expr.attrHandle[ name ],
				ret = isXML ?
					undefined :
					/* jshint eqeqeq: false */
					(tcCore.expr.attrHandle[ name ] = undefined) !=
						getter( elem, name, isXML ) ?

						name.toLowerCase() :
						null;
			tcCore.expr.attrHandle[ name ] = fn;
			return ret;
		} :
		function( elem, name, isXML ) {
			return isXML ?
...
if ( !getSetAttribute ) {
	...
	tcCore.expr.attrHandle.id = tcCore.expr.attrHandle.name = tcCore.expr.attrHandle.coords =
...
// http://msdn.microsoft.com/en-us/library/ms536429%28VS.85%29.aspx
if ( !tcCore.support.hrefNormalized ) {
...
tcCore.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		if ( !(eventHandle = elemData.handle) ) {
			eventHandle = elemData.handle = function( e ) {
				...
				return typeof tcCore !== core_strundefined && (!e || tcCore.event.triggered !== e.type) ?
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		while ( (cur = eventPath[i++]) && !event.isPropagationStopped() ) {
			...
			if ( handle && tcCore.acceptData( cur ) && handle.apply && handle.apply( cur, data ) === false ) {
...
if ( !tcCore.support.submitBubbles ) {
...
if ( !tcCore.support.changeBubbles ) {
...
if ( !tcCore.support.focusinBubbles ) {
...
var isSimple = /^.[^:#\[\.,]*$/,
	...
	rneedsContext = tcCore.expr.match.needsContext,
...
tcCore.fn.extend({
	...
	closest: function( selectors, context ) {
		...
		return this.pushStack( ret.length > 1 ? tcCore.unique( ret ) : ret );
	},
	...
	add: function( selector, context ) {
		var set = typeof selector === "string" ?
				tcCore( selector, context ) :
				tcCore.makeArray( selector && selector.nodeType ? [ selector ] : selector ),
			all = tcCore.merge( this.get(), set );

		return this.pushStack( tcCore.unique(all) );
	},
...
var nodeNames = "abbr|article|aside|audio|bdi|canvas|data|datalist|details|figcaption|figure|footer|" +
	...
	rinlinejQuery = / tcCore\d+="(?:null|\d+)"/g,
	...
	wrapMap = {
		...
		_default: tcCore.support.htmlSerialize ? [ 0, "", "" ] : [ 1, "X<div>", "</div>"  ]
	},
...
tcCore.fn.extend({
	text: function( value ) {
		return tcCore.access( this, function( value ) {
	...
	html: function( value ) {
		return tcCore.access( this, function( value ) {
			...
			if ( typeof value === "string" && !rnoInnerhtml.test( value ) &&
				( tcCore.support.htmlSerialize || !rnoshimcache.test( value )  ) &&
				( tcCore.support.leadingWhitespace || !rleadingWhitespace.test( value ) ) &&
	...
	replaceWith: function() {
		var
			...
			args = tcCore.map( this, function( elem ) {
	...
	domManip: function( args, callback, allowIntersection ) {
		...
		if ( isFunction || !( l <= 1 || typeof value !== "string" || tcCore.support.checkClone || !rchecked.test( value ) ) ) {
		...
		if ( l ) {
			fragment = tcCore.buildFragment( args, this[ 0 ].ownerDocument, false, !allowIntersection && this );
...
function manipulationTarget( elem, content ) {
	return tcCore.nodeName( elem, "table" ) &&
		tcCore.nodeName( content.nodeType === 1 ? content : content.firstChild, "tr" ) ?
...
function fixCloneNodeIssues( src, dest ) {
	...
	if ( !tcCore.support.noCloneEvent && dest[ tcCore.expando ] ) {
	...
	// IE10 throws NoModificationAllowedError if parent is null, #12132.
		...
		if ( tcCore.support.html5Clone && ( src.innerHTML && !tcCore.trim(dest.innerHTML) ) ) {
...
tcCore.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		...
		if ( tcCore.support.html5Clone || tcCore.isXMLDoc(elem) || !rnoshimcache.test( "<" + elem.nodeName + ">" ) ) {
		...
		if ( (!tcCore.support.noCloneEvent || !tcCore.support.noCloneChecked) &&
	...
	buildFragment: function( elems, context, scripts, selection ) {
		...
		for ( ; i < l; i++ ) {
			...
			if ( elem || elem === 0 ) {
				...
				// Convert html into DOM nodes
					...
					if ( !tcCore.support.leadingWhitespace && rleadingWhitespace.test( elem ) ) {
						nodes.push( context.createTextNode( rleadingWhitespace.exec( elem )[0] ) );
					}

					// Remove IE's autoinserted <tbody> from table fragments
					if ( !tcCore.support.tbody ) {
		...
		if ( !tcCore.support.appendChecked ) {
	...
	cleanData: function( elems, /* internal */ acceptData ) {
		var elem, type, id, data,
			...
			internalKey = tcCore.expando,
			cache = tcCore.cache,
			deleteExpando = tcCore.support.deleteExpando,
	...
	for ( ; index < length; index++ ) {
		...
		if ( show ) {
			...
			if ( elem.style.display === "" && isHidden( elem ) ) {
				values[ index ] = tcCore._data( elem, "olddisplay", css_defaultDisplay(elem.nodeName) );
			}
		} else {
...
tcCore.fn.extend({
	css: function( name, value ) {
		return tcCore.access( this, function( elem, name, value ) {
			var len, styles,
				map = {},
				i = 0;
...
tcCore.extend({
	...
	cssProps: {
		...
		"float": tcCore.support.cssFloat ? "cssFloat" : "styleFloat"
	},
	...
	style: function( elem, name, value, extra ) {
		...
		if ( value !== undefined ) {
			...
			if ( !tcCore.support.clearCloneStyle && value === "" && name.indexOf("background") === 0 ) {
				style[ name ] = "inherit";
			}
...
function getWidthOrHeight( elem, name, extra ) {
	...
	var valueIsBorderBox = true,
		...
		styles = getStyles( elem ),
		isBorderBox = tcCore.support.boxSizing && tcCore.css( elem, "boxSizing", false, styles ) === "border-box";
	...
	if ( val <= 0 || val == null ) {
		...
		valueIsBorderBox = isBorderBox && ( tcCore.support.boxSizingReliable || val === elem.style[ name ] );
...
function css_defaultDisplay( nodeName ) {
	...
	if ( !display ) {
		...
		if ( display === "none" || !display ) {
			...
			iframe = ( iframe ||
				tcCore("<iframe frameborder='0' width='0' height='0'/>")
...
function actualDisplay( name, doc ) {
	var elem = tcCore( doc.createElement( name ) ).appendTo( doc.body ),
		display = tcCore.css( elem[0], "display" );
	elem.remove();
	return display;
}
	...
	tcCore.cssHooks[ name ] = {
		get: function( elem, computed, extra ) {
			if ( computed ) {
				...
				return elem.offsetWidth === 0 && rdisplayswap.test( tcCore.css( elem, "display" ) ) ?
		...
		set: function( elem, value, extra ) {
			...
			return setPositiveNumber( elem, value, extra ?
				augmentWidthOrHeight(
					...
					name,
					extra,
					tcCore.support.boxSizing && tcCore.css( elem, "boxSizing", false, styles ) === "border-box",
					styles
				) : 0
			);
		}
...
tcCore(function() {
	if ( !tcCore.support.reliableMarginRight ) {
		tcCore.cssHooks.marginRight = {
			get: function( elem, computed ) {
				if ( computed ) {
					// WebKit Bug 13343 - getComputedStyle returns wrong value for margin-right
	...
	if ( !tcCore.support.pixelPosition && tcCore.fn.position ) {
		tcCore.each( [ "top", "left" ], function( i, prop ) {
			tcCore.cssHooks[ prop ] = {
				get: function( elem, computed ) {
					if ( computed ) {
...
if ( tcCore.expr && tcCore.expr.filters ) {
	tcCore.expr.filters.hidden = function( elem ) {
		// Support: Opera <= 12.12
		// Opera reports offsetWidths and offsetHeights less than zero on some elements
		return elem.offsetWidth <= 0 && elem.offsetHeight <= 0 ||
			(!tcCore.support.reliableHiddenOffsets && ((elem.style && elem.style.display) || tcCore.css( elem, "display" )) === "none");
	};
...
tcCore.extend({
	...
	ajax: function( url, options ) {
		...
		s.dataTypes = tcCore.trim( s.dataType || "*" ).toLowerCase().match( core_rnotwhite ) || [""];
...
tcCore.ajaxSetup({
	...
	jsonpCallback: function() {
		var callback = oldCallbacks.pop() || ( tcCore.expando + "_" + ( ajax_nonce++ ) );
		this[ callback ] = true;
		return callback;
	}
});
...
tcCore.ajaxSettings.xhr = window.ActiveXObject ?
...
tcCore.support.cors = !!xhrSupported && ( "withCredentials" in xhrSupported );
xhrSupported = tcCore.support.ajax = !!xhrSupported;

// Create transport if the browser can provide an xhr
if ( xhrSupported ) {

	tcCore.ajaxTransport(function( s ) {
		// Cross domain only allowed if supported through XMLHttpRequest
		if ( !s.crossDomain || tcCore.support.cors ) {
				...
				send: function( headers, complete ) {
					...
					if ( !s.async ) {
						...
						if ( xhrOnUnloadAbort ) {
							...
							if ( !xhrCallbacks ) {
								xhrCallbacks = {};
								tcCore( window ).unload( xhrOnUnloadAbort );
							}
...
function defaultPrefilter( elem, props, opts ) {
	...
	if ( elem.nodeType === 1 && ( "height" in props || "width" in props ) ) {
		...
		if ( tcCore.css( elem, "display" ) === "inline" &&
				tcCore.css( elem, "float" ) === "none" ) {

			// inline-level elements accept inline-block;
			// block-level elements need to be inline with layout
			if ( !tcCore.support.inlineBlockNeedsLayout || css_defaultDisplay( elem.nodeName ) === "inline" ) {
	...
	if ( opts.overflow ) {
		...
		if ( !tcCore.support.shrinkWrapBlocks ) {
			anim.always(function() {
...
tcCore.fx.timer = function( timer ) {
	if ( timer() && tcCore.timers.push( timer ) ) {
		tcCore.fx.start();
	}
};
...
tcCore.fn.offset = function( options ) {
	if ( arguments.length ) {
...
tcCore.offset = {
	...
	setOffset: function( elem, options, i ) {
		var position = tcCore.css( elem, "position" );
		...
		var curElem = tcCore( elem ),
			curOffset = curElem.offset(),
			curCSSTop = tcCore.css( elem, "top" ),
			curCSSLeft = tcCore.css( elem, "left" ),
			calculatePosition = ( position === "absolute" || position === "fixed" ) && tcCore.inArray("auto", [curCSSTop, curCSSLeft]) > -1,
...
tcCore.each( {scrollLeft: "pageXOffset", scrollTop: "pageYOffset"}, function( method, prop ) {
	...
	tcCore.fn[ method ] = function( val ) {
		return tcCore.access( this, function( elem, method, val ) {
...
tcCore.each( { Height: "height", Width: "width" }, function( name, type ) {
	tcCore.each( { padding: "inner" + name, content: type, "": "outer" + name }, function( defaultExtra, funcName ) {
		...
		tcCore.fn[ funcName ] = function( margin, value ) {
			...
			return tcCore.access( this, function( elem, type, value ) {
				var doc;
...
if ( typeof module === "object" && module && typeof module.exports === "object" ) {
	// Expose tcCore as module.exports in loaders that implement the Node
	// module pattern (including browserify). Do not create the global, since
	// the user will be storing it themselves locally, and globals are frowned
	// upon in the Node module world.
	module.exports = tcCore;
} else {
	// Otherwise expose tcCore to the global object as usual
	...
	if ( typeof define === "function" && define.amd ) {
		define( "tcCore", [], function () { return tcCore; } );
	}
}
