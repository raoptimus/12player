...
tcCore.extend({
	...
	isPlainObject: function( obj ) {
		...
		// - DOM nodes
		// - window
		if ( tcCore.type( obj ) !== "object" || obj.nodeType || tcCore.isWindow( obj ) ) {
			return false;
		}
		...
		if ( typeof selector === "string" ) {
			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					// nodes that are no longer in the document #6963
					if ( elem && elem.parentNode ) {
						// Inject the element directly into the tcCore object
						this.length = 1;
						this[0] = elem;
					}
...
tcCore.extend({
	dir: function( elem, dir, until ) {
		...
		while ( (elem = elem[ dir ]) && elem.nodeType !== 9 ) {
			if ( elem.nodeType === 1 ) {
				if ( truncate && tcCore( elem ).is( until ) ) {
					break;
				}
				matched.push( elem );
			}
...
tcCore.fn.extend({
	has: function( target ) {
		var targets = tcCore( target, this ),
			l = targets.length;
	...
	index: function( elem ) {
		...
		if ( typeof elem === "string" ) {
			return indexOf.call( tcCore( elem ), this[ 0 ] );
		}
...
tcCore.each({
	...
	contents: function( elem ) {
		return elem.contentDocument || tcCore.merge( [], elem.childNodes );
	}
}, function( name, fn ) {
	tcCore.fn[ name ] = function( until, selector ) {
		var matched = tcCore.map( this, fn, until );
		...
		if ( selector && typeof selector === "string" ) {
			matched = tcCore.filter( selector, matched );
		}

		if ( this.length > 1 ) {
			// Remove duplicates
			if ( !guaranteedUnique[ name ] ) {
				tcCore.unique( matched );
			}
 ...
 */
tcCore.acceptData = function( owner ) {
	// Accepts only:
	//  - Node
	//    - Node.ELEMENT_NODE
	//    - Node.DOCUMENT_NODE
...
function Data() {
	...
	this.expando = tcCore.expando + Math.random();
}

Data.uid = 1;
Data.accepts = tcCore.acceptData;
...
Data.prototype = {
	key: function( owner ) {
		...
		if ( !unlock ) {
			...
			// Fallback to a less secure definition
				...
				tcCore.extend( owner, descriptor );
			}
		}
	...
	set: function( owner, data, value ) {
		...
		// Handle: [ owner, { properties } ] args
			...
			if ( tcCore.isEmptyObject( cache ) ) {
				tcCore.extend( this.cache[ unlock ], data );
	...
	access: function( owner, key, value ) {
		...
		if ( key === undefined ||
			...
			return stored !== undefined ?
				stored : this.get( owner, tcCore.camelCase(key) );
		}
	...
	remove: function( owner, key ) {
		...
		if ( key === undefined ) {
			...
			if ( tcCore.isArray( key ) ) {
				...
				name = key.concat( key.map( tcCore.camelCase ) );
			} else {
				camel = tcCore.camelCase( key );
...
tcCore.fn.extend({
	data: function( key, value ) {
		...
		return access( this, function( value ) {
			var data,
				camelKey = tcCore.camelCase( key );

			// The calling tcCore object (element matches) is not empty
			// (and therefore has an element appears at this[ 0 ]) and the
			// `value` parameter was not undefined. An empty tcCore object
		...
		if ( elem ) {
			...
			if ( data ) {
				if ( !queue || tcCore.isArray( data ) ) {
					queue = data_priv.access( elem, type, tcCore.makeArray(data) );
				} else {
					queue.push( data );
				}
			}
...
tcCore.event = {
	...
	add: function( elem, types, handler, data, selector ) {
		...
		if ( !(eventHandle = elemData.handle) ) {
			eventHandle = elemData.handle = function( e ) {
				...
				return typeof tcCore !== strundefined && tcCore.event.triggered !== e.type ?
					tcCore.event.dispatch.apply( elem, arguments ) : undefined;
			};
		}
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		if ( !onlyHandlers && !event.isDefaultPrevented() ) {
			...
			if ( (!special._default || special._default.apply( eventPath.pop(), data ) === false) &&
				...
				if ( ontype && tcCore.isFunction( elem[ type ] ) && !tcCore.isWindow( elem ) ) {
	...
	special: {
		...
		click: {
			...
			trigger: function() {
				if ( this.type === "checkbox" && this.click && tcCore.nodeName( this, "input" ) ) {
					this.click();
					return false;
				}
			},
...
tcCore.removeEvent = function( elem, type, handle ) {
...
function cloneCopyEvent( src, dest ) {
	...
	if ( data_user.hasData( src ) ) {
		...
		udataCur = tcCore.extend( {}, udataOld );
		...
		for ( ; i < l; i++ ) {
			...
			if ( elem || elem === 0 ) {
				...
				if ( tcCore.type( elem ) === "object" ) {
					// Support: QtWebKit
					// tcCore.merge because push.apply(_, arraylike) throws
				...
				if ( hasScripts ) {
					...
					for ( i = 0; i < hasScripts; i++ ) {
						...
						if ( rscriptType.test( node.type || "" ) &&
							!data_priv.access( node, "globalEval" ) && tcCore.contains( doc, node ) ) {
							...
							if ( node.src ) {
								...
								tcCore.globalEval( node.textContent.replace( rcleanScript, "" ) );
							}
						}
					}
				}
...
function getWidthOrHeight( elem, name, extra ) {
	...
	var valueIsBorderBox = true,
		...
		isBorderBox = tcCore.css( elem, "boxSizing", false, styles ) === "border-box";
	...
	for ( ; index < length; index++ ) {
		...
		if ( show ) {
			...
			if ( display !== "none" || !hidden ) {
				data_priv.set( elem, "olddisplay", hidden ? display : tcCore.css( elem, "display" ) );
			}
		}
	}
		...
		set: function( elem, value, extra ) {
			...
			return setPositiveNumber( elem, value, extra ?
				augmentWidthOrHeight(
					...
					name,
					extra,
					tcCore.css( elem, "boxSizing", false, styles ) === "border-box",
					styles
				) : 0
			);
		}
...
tcCore.fn.extend({
	...
	removeProp: function( name ) {
		return this.each(function() {
			delete this[ tcCore.propFix[ name ] || name ];
		});
	}
});
	...
	valHooks: {
		...
		select: {
			...
			set: function( elem, value ) {
				...
				while ( i-- ) {
					option = options[ i ];
					if ( (option.selected = tcCore.inArray( option.value, values ) >= 0) ) {
						optionSet = true;
					}
				}
...
tcCore.fn.extend({
	wrapAll: function( html ) {
		...
		if ( this[ 0 ] ) {
			...
			wrap = tcCore( html, this[ 0 ].ownerDocument ).eq( 0 ).clone( true );
...
tcCore.ajaxSettings.xhr = function() {
	try {
...
tcCore.fn.load = function( url, params, callback ) {
	...
	if ( off >= 0 ) {
		selector = tcCore.trim( url.slice( off ) );
		url = url.slice( 0, off );
	}
...
function getWindow( elem ) {
	return tcCore.isWindow( elem ) ? elem : elem.nodeType === 9 && elem.defaultView;
}
